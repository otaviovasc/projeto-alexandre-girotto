class ReservationEmailTemplate < ApplicationRecord
  ...
  validates :name, :trigger_key, :trigger_anchor, :filial_scope, :subject, :body, :whatsapp_body, presence: true
  ...
  def self.ensure_defaults!
    DEFAULTS.each do |attributes|
      attributes = attributes.merge(whatsapp_body: default_whatsapp_body_for(attributes.fetch(:trigger_key)))
      template = find_or_initialize_by(trigger_key: attributes.fetch(:trigger_key))

      if template.persisted?
        if template.respond_to?(:whatsapp_body) && template.whatsapp_body.blank? && attributes[:whatsapp_body].present?
          template.update_columns(whatsapp_body: attributes[:whatsapp_body], updated_at: Time.current)
        end
        next
      end
  ...
  def self.default_whatsapp_body_for(trigger_key)
    DEFAULT_WHATSAPP_BODIES[trigger_key.to_s]
  end
